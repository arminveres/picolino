...
void Paint_DrawImage(const unsigned char* image, UWORD xStart, UWORD yStart, UWORD W_Image,
                     UWORD H_Image) {
    int i, j;
    for (j = 0; j < H_Image; j++) {
        for (i = 0; i < W_Image; i++) {
            ...
            // Using arrays is a property of sequential storage, accessing the original array by
            // algorithm j*W_Image*2 			   Y offset i*2              	   X offset
        }
    }
}

void Paint_DrawImage1(const unsigned char* image, UWORD xStart, UWORD yStart, UWORD W_Image,
                      UWORD H_Image) {
    int i, j;
    for (j = 0; j < H_Image; j++) {
        for (i = 0; i < W_Image; i++) {
    ...
    for (y = 0; y < Paint.HeightByte; y++) {
        for (x = 0; x < Paint.WidthByte; x++) {  // 8 pixel =  1 byte
            ...
            Paint.Image[Addr] = (unsigned char)
                image_buffer[Addr + (Paint.HeightByte) * Paint.WidthByte * (Region - 1)];
        }
    }
}
